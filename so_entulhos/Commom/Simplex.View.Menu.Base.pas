...
procedure TfrmMenuBase.FormCreate(Sender: TObject);
begin
  ItemLista := TObjectList<TFrameMenu>.Create;
  lblData.Caption := formatdatetime('dd "de" MMMM "de" yyyy',now);
end;

procedure TfrmMenuBase.FormDestroy(Sender: TObject);
begin
  FreeAndNil(ItemLista);
end;

procedure TfrmMenuBase.FormShow(Sender: TObject);
var
...
begin
  ...
  Item1.Align := alTop;
//  Item1.PanelInjection('Cliente', TfrmCadastroCliente, pnlTela);
...
procedure TfrmMenuBase.TimerTimer(Sender: TObject);
begin
